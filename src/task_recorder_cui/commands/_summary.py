...
from task_recorder_cui.repo import find_active_record, find_category, row_to_record
from task_recorder_cui.utils.time import format_duration, now_utc, to_iso

_WEEKDAY_EN: list[str] = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
...
def _period_bounds_utc(start_local_date: date, end_local_date: date) -> tuple[datetime, datetime]:
    """ローカル日付範囲を tz付き UTC 半開区間 [start_utc, end_next_utc) に変換。"""
    start_utc = datetime.combine(start_local_date, time.min).astimezone()
    end_next_utc = datetime.combine(end_local_date + timedelta(days=1), time.min).astimezone()
...
def aggregate_period(
    ...
    *,
    ...
    """
    start_utc, end_next_utc = _period_bounds_utc(start_local_date, end_local_date)
    ...
    for key in per_category:
        cat = find_category(conn, key)
        display_names[key] = cat.display_name if cat else key
...
def render_breakdown_table(summary: PeriodSummary, title: str) -> Table:
    ...
    for day in summary.days:
        weekday = _WEEKDAY_EN[day.local_date.weekday()]
