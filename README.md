...
- **日本語 / 英語の切替**: `--lang ja|en` フラグまたは `LANG=en_US.UTF-8` 環境変数で
  UI メッセージを切替可能。デフォルトは日本語。
...
UI メッセージは日本語 / 英語の二言語に対応しています (デフォルトは日本語)。

```bash
tsk --lang en now            # 単発で英語出力
tsk --lang ja now            # 単発で日本語出力
LANG=en_US.UTF-8 tsk today   # 環境変数経由で切替
```

優先順位 (高い順): `--lang` フラグ > `config.toml` の `ui.lang` > `LC_ALL` / `LANG` 環境変数 > `ja`。

なお、カテゴリの `display_name` はユーザ入力値なので翻訳されません (`tsk cat rename <key> "<新表示名>"` で個別に変更してください)。`--help` 出力は現バージョンでは英語のみです。
